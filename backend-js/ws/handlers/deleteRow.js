...
async function handleDeleteRow({ socket, message, reply, broadcastToRoom }) {
  ...
  try {
    const responseData = await sheetStructureService.applyDeleteRow(message);
    const payload = createWsSuccess(WS_MESSAGE_TYPES.ROW_DELETED, responseData);
    reply(payload);
