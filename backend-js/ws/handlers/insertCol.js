...
async function handleInsertCol({ socket, message, reply, broadcastToRoom }) {
  ...
  try {
    const responseData = await sheetStructureService.applyInsertCol(message);
    const payload = createWsSuccess(WS_MESSAGE_TYPES.COL_INSERTED, responseData);
    reply(payload);
...
module.exports = handleInsertCol;
