...
const { commitSetCell } = require('./gate/gateSetCellService');
...
async function applySetCell(command = {}) {
  ...
  return docLock.withDocLock(normalizedCommand.docId, async () => {
    ...
    if (realtimeConfig.driver === 'redis') {
      const result = await commitSetCell(normalizedCommand);
      seq = result.seq;
      targetSheetId = result.targetSheetId;
      oldValue = result.oldValue;
      oldStyle = result.oldStyle;
      ...
      undoStack.push({
        ...
        opType: 'set_cell', sheetId: targetSheetId,
        row: effectiveCommand.row, col: effectiveCommand.col,
        oldValue, oldStyle,
        newValue: effectiveCommand.value, newStyle: effectiveCommand.style,
      ...
      const executeMutation = async (connection = null) => {
        ...
        updatedDoc = await docsService.applySetCell({ ...effectiveCommand }, { connection });
        ...
        targetSheetId = updatedDoc._targetSheetId;
        oldValue = updatedDoc._before.value;
        oldStyle = updatedDoc._before.style;

        await historyStore.append({
          docId: normalizedCommand.docId, clientId: normalizedCommand.clientId,
          seq, baseSeq: rebaseResult.baseSeq, opType: 'set_cell',
          targetSheetId, targetRow: effectiveCommand.row, targetCol: effectiveCommand.col,
    ...
    await auditService.recordAuditEvent({
      type: 'set_cell', docId: normalizedCommand.docId, clientId: normalizedCommand.clientId, seq,
    });
